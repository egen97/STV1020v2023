...
table(FairFPSR3$growth, FairFPSR3$growth_dich, useNA = "always")
...
growth_graf <- ggplot(data = FairFPSR3) +
  geom_bar(aes(x=growth, fill = growth_dich),
           binwidth = 1) +
  theme_classic() +
  theme(legend.title=element_blank()) +
...
ggsave("graf_growthdic.jpeg")
ggsave("hei.png", growth_graf)
...
cor.test(FairFPSR3$growth, FairFPSR3$inc_vote)
...
lm(avhengig_variabel ~ uavhengig_variabel + uavhengig_variabel_2, data = mitt_datasett)
...
stargazer(model, 
          type = "html",
          covariate.labels = c("Vekst", "Konstantledd"),
...
ggplot(data = FairFPSR3) +
  ...
  labs(y = "Incumbent-Party Vote Percentage",
       x = "Percentage change in Real GDP Per Capita") +
  geom_smooth(aes(x = growth, y = inc_vote), method = "lm") 
...
ggplot(FairFPSR3, aes(residuals)) +
  geom_density()
