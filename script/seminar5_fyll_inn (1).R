...
model2 <- lm(inc_vote ~ inflation, 
            data = FairFPSR3)

model3 <- lm(inc_vote ~ growth + inflation, 
             data = FairFPSR3)
...
stargazer(model, model2, model3,  
          type = "text",
          covariate.labels = c("Økonomisk Vekst", "Inflasjon"),
          dep.var.labels = c("Stemmer på sittende president"))
...
# 
# ggplot(FairFPSR3) +
#   geom_bar(aes(growth_dich)


lm(inc_vote ~ year, data = FairFPSR3)
